...
namespace Nzl.Log4Net.Appender
{
	...
	/// the Nzl.Log4Net assembly that is built against the MS .NET 1.1 framework and that you are
	...
	public class SmtpAppender : BufferingAppenderSkeleton
	{
		...
		virtual protected void SendEmail(string messageBody)
		{
            ...
            using (MailMessage mailMessage = new MailMessage())
            {
				...
				{
					ErrorHandler.Error("SmtpAppender: Authentication is only supported on the MS .NET 1.1 or MS .NET 2.0 builds of Nzl.Log4Net");
				}

				if (m_port != 25)
				{
					ErrorHandler.Error("SmtpAppender: Server Port is only supported on the MS .NET 1.1 or MS .NET 2.0 builds of Nzl.Log4Net");
				}
#endif // if NET_1_1
