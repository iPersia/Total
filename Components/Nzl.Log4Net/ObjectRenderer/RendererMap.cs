...
namespace Nzl.Log4Net.ObjectRenderer
{
	/// <summary>
			...
			else 
			{
				...
				else
				{
					...
					catch(Exception ex)
					{
						...
						Nzl.Log4Net.Util.LogLog.Error(declaringType, "Exception while rendering object of type ["+obj.GetType().FullName+"]", ex);
						...
						writer.Write("<Nzl.Log4Net.Error>Exception rendering object type ["+objectTypeName+"]");
						if (ex != null)
						{
						...
						writer.Write("</Nzl.Log4Net.Error>");
					}
				}
			}
		}
