...
using Nzl.Log4Net.ObjectRenderer;

namespace Nzl.Log4Net.Repository.Hierarchy
{
	/// <summary>
	/// Initializes the Nzl.Log4Net environment using an XML DOM.
	/// </summary>
	/// <remarks>
	/// <para>
	...
	public class XmlHierarchyConfigurator
	{
				...
				foreach (XmlNode currentNode in appenderElement.ChildNodes)
				{
					...
					if (currentNode.NodeType == XmlNodeType.Element) 
					{
						...
						if (currentElement.LocalName == APPENDER_REF_TAG)
						{
							...
							{
								LogLog.Error(declaringType, "Requesting attachment of appender named ["+refName+ "] to appender named [" + appender.Name + "] which does not implement Nzl.Log4Net.Core.IAppenderAttachable.");
							}
						}
						else
						{
		...
		{
			// Create a new Nzl.Log4Net.Logger object from the <logger> element.
			string loggerName = loggerElement.GetAttribute(NAME_ATTR);

			LogLog.Debug(declaringType, "Retrieving an instance of Nzl.Log4Net.Repository.Logger for logger [" + loggerName + "].");
		...
		private const string CONFIGURATION_TAG			= "Nzl.Log4Net";
