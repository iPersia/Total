...
namespace Nzl.Log4Net.Repository.Hierarchy
{
	/// <summary>
	...
	/// </para>
	/// <para>
	/// This is one of the central classes in the Nzl.Log4Net implementation. One of the
	/// distinctive features of Nzl.Log4Net are hierarchical loggers and their
	...
	public abstract class Logger : IAppenderAttachable, ILogger
	{
		...
		virtual public void AddAppender(IAppender newAppender) 
		{
			...
			try
			{
				...
				{
					m_appenderAttachedImpl = new Nzl.Log4Net.Util.AppenderAttachedImpl();
				}
		...
		virtual public void Log(Type callerStackBoundaryDeclaringType, Level level, object message, Exception exception) 
		{
			...
			{
				Nzl.Log4Net.Util.LogLog.Error(declaringType, "Exception while logging", ex);
			}
#if !NET_2_0 && !MONO_2_0
			catch
			{
				Nzl.Log4Net.Util.LogLog.Error(declaringType, "Exception while logging");
			}
#endif
		}
		...
		virtual protected void CallAppenders(LoggingEvent loggingEvent) 
		{
			...
			if (!m_hierarchy.EmittedNoAppenderWarning && writes == 0) 
			{
				...
				LogLog.Debug(declaringType, "Please initialize the Nzl.Log4Net system properly.");
				try
				{
		...
		/// </summary>
		private Nzl.Log4Net.Util.AppenderAttachedImpl m_appenderAttachedImpl;
