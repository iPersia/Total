...
{
	/// <summary>
	/// Use this class to initialize the Nzl.Log4Net environment using an Xml tree.
	/// </summary>
	/// <remarks>
	/// <para>
	...
	public sealed class XmlConfigurator
	{
...
#if !NETCF
		/// <summary>
		/// Automatically configures the Nzl.Log4Net system based on the 
		...
		/// <c>Nzl.Log4Net</c> that contains the configuration data.
		/// </para>
		/// <para>
		/// To use this method to configure Nzl.Log4Net you must specify 
		/// the <see cref="Log4NetConfigurationSectionHandler"/> section
		/// handler for the <c>Nzl.Log4Net</c> configuration section. See the
	    ...
	    static private void InternalConfigure(ILoggerRepository repository) 
		{
...
#if NET_2_0
				configElement = System.Configuration.ConfigurationManager.GetSection("Nzl.Log4Net") as XmlElement;
#else
				configElement = System.Configuration.ConfigurationSettings.GetConfig("Nzl.Log4Net") as XmlElement;
#endif
				if (configElement == null)
				{
					// Failed to load the xml config using configuration settings handler
					LogLog.Error(declaringType, "Failed to find configuration section 'Nzl.Log4Net' in the application's .config file. Check your .config file for the <Nzl.Log4Net> and <configSections> elements. The configuration section should look like: <section name=\"Nzl.Log4Net\" type=\"Nzl.Log4Net.Config.Log4NetConfigurationSectionHandler,Nzl.Log4Net\" />");
				}
				else
				{
			...
			catch(System.Configuration.ConfigurationException confEx)
			{
				...
				else
				{
					...
					string configSectionStr = "<section name=\"Nzl.Log4Net\" type=\"Nzl.Log4Net.Config.Log4NetConfigurationSectionHandler," + Assembly.GetExecutingAssembly().FullName + "\" />";
		...
		/// <summary>
		/// Configures Nzl.Log4Net using a <c>Nzl.Log4Net</c> element
		/// </summary>
		/// <remarks>
		/// <para>
		/// Loads the Nzl.Log4Net configuration from the XML element
...
#if !NETCF
		/// <summary>
		/// Configures Nzl.Log4Net using the specified configuration file.
		/// </summary>
		/// <param name="configFile">The XML file to load the configuration from.</param>
		/// <remarks>
		/// <para>
		/// The configuration file must be valid XML. It must contain
		/// at least one element called <c>Nzl.Log4Net</c> that holds
		/// the Nzl.Log4Net configuration data.
		/// </para>
		/// <para>
		/// The Nzl.Log4Net configuration file can possible be specified in the application's
		...
		/// a configuration section for the <c>Nzl.Log4Net</c> element otherwise .NET will 
		...
		/// </para>
		/// <example>
		/// The following example configures Nzl.Log4Net using a configuration file, of which the 
		...
		/// ...
		/// 
		/// XmlConfigurator.Configure(new FileInfo(ConfigurationSettings.AppSettings["Nzl.Log4Net-config-file"]));
		/// </code>
		/// <para>
		/// In the <c>.config</c> file, the path to the Nzl.Log4Net can be specified like this :
		/// </para>
		/// <code lang="XML" escaped="true">
		/// <configuration>
		///		<appSettings>
		///			<add key="Nzl.Log4Net-config-file" value="log.config"/>
		///		</appSettings>
		///	</configuration>
		/// </code>
		/// </remarks>
		...
		/// <summary>
		/// Configures Nzl.Log4Net using the specified configuration URI.
		/// </summary>
		...
		/// <summary>
		/// Configures Nzl.Log4Net using the specified configuration data stream.
		/// </summary>
		...
		static private void InternalConfigure(ILoggerRepository repository, Stream configStream)
		{
			...
			else
			{
				...
				if (doc != null)
				{
					...
					// Configure using the 'Nzl.Log4Net' element
					XmlNodeList configNodeList = doc.GetElementsByTagName("Nzl.Log4Net");
					if (configNodeList.Count == 0)
					{
						LogLog.Debug(declaringType, "XML configuration does not contain a <Nzl.Log4Net> element. Configuration Aborted.");
					}
					else if (configNodeList.Count > 1)
					{
						LogLog.Error(declaringType, "XML configuration contains [" + configNodeList.Count + "] <Nzl.Log4Net> elements. Only one is allowed. Configuration Aborted.");
					}
					else
					{
		...
		/// <summary>
		/// Configures Nzl.Log4Net using the file specified, monitors the file for changes 
		...
		/// </para>
		/// <para>
		/// For more information on how to configure Nzl.Log4Net using
		...
		/// <summary>
		/// Configures the specified repository using a <c>Nzl.Log4Net</c> element.
		/// </summary>
