...
using Nzl.Log4Net.Config;
using Nzl.Log4Net.Util;
using Nzl.Log4Net.Repository;

namespace Nzl.Log4Net.Core
{
	/// <summary>
	...
	public class DefaultRepositorySelector : IRepositorySelector
	{
		...
		/// an appropriate value would be <see cref="Nzl.Log4Net.Repository.Hierarchy.Hierarchy"/>.
		/// </para>
		/// </remarks>
		...
		public DefaultRepositorySelector(Type defaultRepositoryType)
		{
			...
			{
				throw Nzl.Log4Net.Util.SystemInfo.CreateArgumentOutOfRangeException("defaultRepositoryType", defaultRepositoryType, "Parameter: defaultRepositoryType, Value: [" + defaultRepositoryType + "] out of range. Argument must implement the ILoggerRepository interface");
			}
		...
		/// to create can be overridden by specifying the <see cref="Nzl.Log4Net.Config.RepositoryAttribute"/> 
		/// attribute on the <paramref name="repositoryAssembly"/>.
		/// </para>
		/// <para>
		/// The default values are to use the <see cref="Nzl.Log4Net.Repository.Hierarchy.Hierarchy"/> 
		...
		/// any <see cref="Nzl.Log4Net.Config.ConfiguratorAttribute"/> attributes defined on
		...
		/// <see cref="Nzl.Log4Net.Config.RepositoryAttribute"/> attribute on the 
		...
		/// configured using any <see cref="Nzl.Log4Net.Config.ConfiguratorAttribute"/> 
		...
		/// Also the <see cref="Nzl.Log4Net.Config.RepositoryAttribute"/> attribute on the
		...
		/// </summary>
		/// <param name="assembly">The assembly that has a <see cref="Nzl.Log4Net.Config.RepositoryAttribute"/>.</param>
		...
		private void GetInfoForAssembly(Assembly assembly, ref string repositoryName, ref Type repositoryType)
		{
			...
			try
			{
				...
				object[] repositoryAttributes = Attribute.GetCustomAttributes(assembly, typeof(Nzl.Log4Net.Config.RepositoryAttribute), false);
				...
				else
				{
					...
					{
						LogLog.Error(declaringType, "Assembly [" + assembly + "] has multiple Nzl.Log4Net.Config.RepositoryAttribute assembly attributes. Only using first occurrence.");
					}

					Nzl.Log4Net.Config.RepositoryAttribute domAttr = repositoryAttributes[0] as Nzl.Log4Net.Config.RepositoryAttribute;
		...
		/// </summary>
		/// <param name="assembly">The assembly containing <see cref="Nzl.Log4Net.Config.ConfiguratorAttribute"/>
		...
		private void ConfigureRepository(Assembly assembly, ILoggerRepository repository)
		{
			...
			object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(Nzl.Log4Net.Config.ConfiguratorAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0)
			{
				...
				foreach(Nzl.Log4Net.Config.ConfiguratorAttribute configAttr in configAttributes)
				{
					if (configAttr != null)
					{
						try
			...
			if (repository.Name == DefaultRepositoryName)
			{
				...
				string repositoryConfigFile = SystemInfo.GetAppSetting("Nzl.Log4Net.Config");
				if (repositoryConfigFile != null && repositoryConfigFile.Length > 0)
				{
					...
					{
						LogLog.Warn(declaringType, "Exception getting ApplicationBaseDirectory. appSettings Nzl.Log4Net.Config path ["+repositoryConfigFile+"] will be treated as an absolute URI", ex);
					}
...
#if NET_2_0 || MONO_2_0
				    Boolean.TryParse(SystemInfo.GetAppSetting("Nzl.Log4Net.Config.Watch"), out watchRepositoryConfigFile);
#else
                                    {
                                        string watch = SystemInfo.GetAppSetting("Nzl.Log4Net.Config.Watch");
					...
					if (watchRepositoryConfigFile)
					{
						...
						{
                            LogLog.Error(declaringType, "DefaultRepositorySelector: Exception while parsing Nzl.Log4Net.Config file physical path [" + repositoryConfigFilePath + "]", ex);
						}
						try
						{
					...
					{
						LogLog.Error(declaringType, "Exception while parsing Nzl.Log4Net.Config file path ["+repositoryConfigFile+"]", ex);
					}
		...
		private void LoadPlugins(Assembly assembly, ILoggerRepository repository)
		{
			...
			object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(Nzl.Log4Net.Config.PluginAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0)
			{
				foreach(Nzl.Log4Net.Plugin.IPluginFactory configAttr in configAttributes)
				{
					try
					{
		...
		private void LoadAliases(Assembly assembly, ILoggerRepository repository)
		{
			...
			object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(Nzl.Log4Net.Config.AliasRepositoryAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0)
			{
				foreach(Nzl.Log4Net.Config.AliasRepositoryAttribute configAttr in configAttributes)
				{
					try
					{
		...
		private const string DefaultRepositoryName = "Nzl.Log4Net-default-repository";
