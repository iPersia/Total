...
namespace Nzl.Log4Net.Util
{
	...
	public sealed class SystemStringFormat
	{
		...
		private static string StringFormat(IFormatProvider provider, string format, params object[] args)
		{
			...
			{
				Nzl.Log4Net.Util.LogLog.Warn(declaringType, "Exception while rendering format ["+format+"]", ex);
				return StringFormatError(ex, format, args);
			}
#if !NET_2_0 && !MONO_2_0
			catch
			{
				Nzl.Log4Net.Util.LogLog.Warn(declaringType, "Exception while rendering format ["+format+"]");
		...
		private static string StringFormatError(Exception formatException, string format, object[] args)
		{
			try
			{
				StringBuilder buf = new StringBuilder("<Nzl.Log4Net.Error>");
				...
				buf.Append("</args>");
				buf.Append("</Nzl.Log4Net.Error>");

				return buf.ToString();
			}
			catch(Exception ex)
			{
				Nzl.Log4Net.Util.LogLog.Error(declaringType, "INTERNAL ERROR during StringFormat error handling", ex);
				return "<Nzl.Log4Net.Error>Exception during StringFormat. See Internal Log.</Nzl.Log4Net.Error>";
			}
#if !NET_2_0 && !MONO_2_0
			catch
			{
				Nzl.Log4Net.Util.LogLog.Error(declaringType, "INTERNAL ERROR during StringFormat error handling");
				return "<Nzl.Log4Net.Error>Exception during StringFormat. See Internal Log.</Nzl.Log4Net.Error>";
			}
#endif
		}
