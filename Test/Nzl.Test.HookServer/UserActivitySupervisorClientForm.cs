...
namespace Nzl.Test.Hook
{
    public partial class UserActivitySupervisorClientForm : Form
    {
        ...
        /// 
        /// </summary>
        private string _uasWindowName = "User Activity Supervisor Server";
        ...
        private void uiTimer_Tick(object sender, EventArgs e)
        {
            try
            {
                if (this._sharedMemory != null)
                {
                    ...
                    char[] logLen = new char[8];
                    this._sharedMemory.Read(ref logLen, 0, 8);
                    if (logLen[0] != '\0')
                    {
                        int logInforLength = System.Convert.ToInt32(new string(logLen).TrimEnd());
                        if (logInforLength < 102400 && logInforLength > 0)
                        {
                            char[] loginfo = new char[logInforLength];
                            this._sharedMemory.Read(ref loginfo, 0, loginfo.Length);
                            Update(new string(loginfo).Substring(8));
                        }
                    }
                }
            }
        ...
        public string FormatStrings(string[] strs, int[] lens, LeftRightAlignment[] lrAlgs, char replacement)
        {
            if (strs.Length == lens.Length && strs.Length == lrAlgs.Length)
            {
                ...
                for (int i = 0; i < strs.Length; i++)
                {
                    ...
                    {
                        result += strs[i].PadRight(lens[i], replacement);
                    }
                    else
                    {
                        result += strs[i].PadLeft(lens[i], replacement);
                    }
                }
