namespace Nzl.Web.Pub.MobileNewSmth
{
    ...
    /// The topic form.
    /// </summary>
    public partial class TopicSrcForm : Form
    {
        #region Variable
        /// <summary>
        /// 
        ...
        public TopicSrcForm(string uri)
            : this()
        {
            this._topicUrl = uri;
        }
        ...
        /// </summary>
        /// <param name="uri"></param>
        public TopicSrcForm(string uri, string userID)
            : this(uri)
        {
        ...
        private IList<Thread> GetThreads(string html)
        {
            string pattern = @"<div class=\Wnav hl\W><div><a class=\Wplant\W>"
                           ...
                           + @"(?'QueryType'只看此ID|展开)</a></div><div>(<a href=\W"
        ...
        /// <param name="page"></param>
        /// <returns></returns>
        private void GetPageInfo(WebPage page)
        {
        ...
        private IList<string> GetImageUrls(ref string content)
        {
            ...
            if (mtCollection != null)
            {
                foreach (Match mt in mtCollection)
                {
                    ...
                    content = content.Replace(mt.Groups[0].Value.ToString(), TopicSrcForm._imageToken);
                }
            }
        ...
        /// </summary>
        /// <param name="html"></param>
        private void GetThreadActionInfo(string html)
        {
        ...
        /// 
        /// </summary>
        private void UpdateText(WebPage wp)
        {
        ...
        private void AddAllReplyContentToRichTextBox(RichTextBox richTextBox, Thread thread)
        {
            if (richTextBox != null && thread != null)
            {
                ...
                {
                    MatchCollection mtCollection = CommonUtil.GetMatchCollection(TopicSrcForm._imageToken, thread.Content);
                    int counter = 0;
                    ...
                    foreach (Image image in thread.ImageList)
                    {
                        ...
                        content = content.Substring(pos + TopicSrcForm._imageToken.Length);
                ...
                else
                {
                    ...
                    if (wp != null && wp.IsGood)
                    {
                        this.Invoke((EventHandler)delegate
                        {
                            this.UpdateText(wp);
                            this.GetPageInfo(wp);
                            this.GetThreadActionInfo(wp.Html);
